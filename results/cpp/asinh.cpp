...
std::complex<float> asinh_2(std::complex<float> z) {
  ...
  float xoy = (((y_gt_safe_max_opt) &&
                (!((y) == (std::numeric_limits<float>::infinity()))))
                   ...
                   : (zero));
  float one = 1.0;
  bool logical_and_328 = ((y) < (4.3368087e-19)) && ((x) < (one));
  float xp1 = (x) + (one);
  float xm1 = (x) - (one);
...
std::complex<double> asinh_3(std::complex<double> z) {
  ...
  double xoy = (((y_gt_safe_max_opt) &&
                 (!((y) == (std::numeric_limits<double>::infinity()))))
                    ...
                    : (zero));
  double one = 1.0;
  bool logical_and_328 = ((y) < (5.966672584960166e-154)) && ((x) < (one));
  double xp1 = (x) + (one);
  double xm1 = (x) - (one);

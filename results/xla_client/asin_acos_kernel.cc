...
XlaOp asin_acos_kernel_0(XlaOp z) {
  ...
  XlaOp mx__1 = Select(y_gt_safe_max_opt, y, x);
  XlaOp xoy = Select(
      ...
      Select(Ge(Max(x, y), safe_max), y,
             Select(Le(x, one), Sqrt(Mul(half_apx, Add(Div(yy, rpxp1), smxm1))),
                    Mul(y, Sqrt(Add(Div(half_apx, rpxp1),
                                    Div(half_apx, spxm1)))))),
      Select(Ge(mx__1, Select(y_gt_safe_max_opt, safe_max_opt, safe_max)),
             Add(Add(ScalarLike(signed_x, std::log(two_)), Log(mx__1)),

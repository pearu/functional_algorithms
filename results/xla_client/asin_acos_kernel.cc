...
XlaOp asin_acos_kernel_0(XlaOp z) {
  ...
  XlaOp _mx_1_ = Select(y_gt_safe_max_opt, y, x);
  XlaOp xoy = Select(
      And(y_gt_safe_max_opt,
          Not(Eq(y, ScalarLike(signed_y,
                               std::numeric_limits<FloatType>::infinity())))),
      Div(x, y), zero);
  XlaOp logical_and_69 = And(
      Lt(y,
         ScalarLike(signed_x,
  ...
  XlaOp am1 =
      Select(logical_and_69, Neg(Div(Mul(xp1, xm1), ap1)), x_ge_1_or_not);
      ...
      Select(Ge(Max(x, y), safe_max), y,
             Select(Le(x, one), Sqrt(Mul(half_apx, Add(Div(yy, rpxp1), smxm1))),
                    Mul(y, Sqrt(Add(Div(half_apx, rpxp1),
                                    Div(half_apx, spxm1)))))),
      Select(Ge(_mx_1_, Select(y_gt_safe_max_opt, safe_max_opt, safe_max)),
             Add(Add(ScalarLike(signed_x, std::log(two_)), Log(_mx_1_)),
                 Mul(half, Log1p(Mul(xoy, xoy)))),
             Select(logical_and_69, Div(y, sq), Log1p(Add(am1, sq)))));
}
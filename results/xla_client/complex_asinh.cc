...
XlaOp complex_asinh_0(XlaOp z) {
  XlaOp _asin_acos_kernel_1_signed_y = Real(z);
  FloatType zero_ = 0;
  XlaOp y = Abs(_asin_acos_kernel_1_signed_y);
  XlaOp signed_y = Imag(z);
  XlaOp x = Abs(Neg(signed_y));
  FloatType safe_max_ =
      (std::sqrt(std::numeric_limits<FloatType>::max())) / (8);
  XlaOp safe_max_opt =
      Select(Lt(x, ScalarLike(signed_y, (safe_max_) * (1000000000000.0))),
             ScalarLike(signed_y, (safe_max_) * (1e-06)),
             ScalarLike(signed_y, (safe_max_) * (100.0)));
  XlaOp y_gt_safe_max_opt = Ge(y, safe_max_opt);
  XlaOp mx = Select(y_gt_safe_max_opt, y, x);
  XlaOp safe_max = ScalarLike(signed_y, safe_max_);
  FloatType two_ = 2;
  XlaOp half = ScalarLike(signed_y, 0.5);
  XlaOp zero = ScalarLike(signed_y, zero_);
  XlaOp xoy = Select(
      And(y_gt_safe_max_opt,
          Not(Eq(y, ScalarLike(_asin_acos_kernel_1_signed_y,
                               std::numeric_limits<FloatType>::infinity())))),
      Div(x, y), zero);
  XlaOp one = ScalarLike(signed_y, 1);
  ...
  XlaOp _asin_acos_kernel_1_imag =
  ...
  return Complex(
      Select(Lt(_asin_acos_kernel_1_signed_y,
                ScalarLike(_asin_acos_kernel_1_signed_y, zero_)),
             Neg(_asin_acos_kernel_1_imag), _asin_acos_kernel_1_imag),
      Atan2(signed_y,

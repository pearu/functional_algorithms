...
XlaOp asinh_1(XlaOp z) {
  ...
  XlaOp _signed_y_0_ = Imag(z);
  XlaOp x = Abs(Neg(_signed_y_0_));
  FloatType safe_max_ =
      (std::sqrt(std::numeric_limits<FloatType>::max())) / (8);
  XlaOp safe_max_opt =
      Select(Lt(x, ScalarLike(_signed_y_0_, (safe_max_) * (1000000000000.0))),
             ScalarLike(_signed_y_0_, (safe_max_) * (1e-06)),
             ScalarLike(_signed_y_0_, (safe_max_) * (100.0)));
  XlaOp y_gt_safe_max_opt = Ge(y, safe_max_opt);
  XlaOp mx = Select(y_gt_safe_max_opt, y, x);
  XlaOp safe_max = ScalarLike(_signed_y_0_, safe_max_);
  FloatType two_ = 2;
  XlaOp half = ScalarLike(_signed_y_0_, 0.5);
  XlaOp zero = ScalarLike(_signed_y_0_, zero_);
  XlaOp xoy = Select(
      And(y_gt_safe_max_opt,
          Not(Eq(y, ScalarLike(signed_y,
                               std::numeric_limits<FloatType>::infinity())))),
      Div(x, y), zero);
  XlaOp one = ScalarLike(_signed_y_0_, 1);
  XlaOp logical_and_278 = And(
      Lt(y,
         ScalarLike(_signed_y_0_,
  ...
  XlaOp sqrt_two = ScalarLike(_signed_y_0_, std::sqrt(2));
  XlaOp _r_0_ = Square(Div(mn, _mx_0_));
  XlaOp sqa = Sqrt(Add(one, _r_0_));
  XlaOp two = ScalarLike(_signed_y_0_, two_);
  XlaOp r = Select(
  ...
  XlaOp x_ge_1_or_not = Select(
      ...
      Select(Le(a, ScalarLike(_signed_y_0_, 1.5)),
             Add(divide_half_yy_rpxp1, Div(half_yy, smxm1)), Sub(a, one)));
  XlaOp am1 =
      Select(logical_and_278, Neg(Div(Mul(xp1, xm1), ap1)), x_ge_1_or_not);
  XlaOp sq = Sqrt(Mul(am1, ap1));
  XlaOp _imag_0_ =
      Select(Ge(mx, Select(y_gt_safe_max_opt, safe_max_opt, safe_max)),
             Add(Add(ScalarLike(_signed_y_0_, std::log(two_)), Log(mx)),
                 Mul(half, Log1p(Mul(xoy, xoy)))),
             Select(logical_and_278, Div(y, sq), Log1p(Add(am1, sq))));

...
def asin_1(z: numpy.complex64) -> numpy.complex64:
  with warnings.catch_warnings(action="ignore"):
    ...
    r: numpy.float32 = ((_hypot_1_mx) * (sqrt_two)) if (numpy.equal(_hypot_1_mx, mn, dtype=numpy.bool_)) else ((_hypot_1_mx) * (numpy.sqrt(((_square_1_z) * (_square_1_z)) + (numpy.float32(1)))))
    ...
    s: numpy.float32 = ((_hypot_2_mx) * (sqrt_two)) if (numpy.equal(_hypot_2_mx, _hypot_2_mn, dtype=numpy.bool_)) else ((_hypot_2_mx) * (numpy.sqrt(((_square_2_z) * (_square_2_z)) + (numpy.float32(1)))))
    ...
    real: numpy.float32 = numpy.arctan2(signed_x, (y) if ((max(x, y)) >= (safe_max)) else ((numpy.sqrt((half_apx) * (((yy) / (rpxp1)) + (smxm1)))) if ((x) <= (numpy.float32(1))) else ((y) * (numpy.sqrt(((half_apx) / (rpxp1)) + ((half_apx) / (spxm1)))))))
    safe_max_opt: numpy.float32 = ((safe_max) * (numpy.float32(1e-06))) if ((x) < ((safe_max) * (numpy.float32(1000000000000.0)))) else ((safe_max) * (numpy.float32(100.0)))
    y_gt_safe_max_opt: numpy.bool_ = (y) >= (safe_max_opt)
    mx: numpy.float32 = (y) if (y_gt_safe_max_opt) else (x)
    posinf: numpy.float32 = numpy.float32(numpy.inf)
    xoy: numpy.float32 = ((x) / (y)) if ((y_gt_safe_max_opt) and (not (numpy.equal(y, posinf, dtype=numpy.bool_)))) else (numpy.float32(0))
    smallest: numpy.float32 = finfo_float32.tiny
    logical_and_lt_y_safe_min_lt_x_one: numpy.bool_ = ((y) < ((numpy.sqrt(smallest)) * (numpy.float32(4)))) and ((x) < (numpy.float32(1)))
    ...
    x_ge_1_or_not: numpy.float32 = ((divide_half_yy_rpxp1) + ((numpy.float32(0.5)) * (spxm1))) if ((x) >= (numpy.float32(1))) else (((divide_half_yy_rpxp1) + ((half_yy) / (smxm1))) if ((a) <= (numpy.float32(1.5))) else ((a) - (numpy.float32(1))))
    ...
    imag: numpy.float32 = (((numpy.log(numpy.float32(2))) + (numpy.log(mx))) + ((numpy.float32(0.5)) * (numpy.log1p((xoy) * (xoy))))) if ((mx) >= ((safe_max_opt) if (y_gt_safe_max_opt) else (safe_max))) else (((y) / (sq)) if (logical_and_lt_y_safe_min_lt_x_one) else (numpy.log1p((am1) + (sq))))

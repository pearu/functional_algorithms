...
def log2_0(z: numpy.complex128) -> numpy.complex128:
    with warnings.catch_warnings(action="ignore"):
        z = numpy.complex128(z)
        lnz: numpy.complex128 = numpy.log(z)
        x: numpy.float64 = (lnz).real
        ln2: numpy.float64 = numpy.log(numpy.float64(2.0))
        result = make_complex((x) / (ln2), ((lnz).imag) / (ln2))
        return result


def log2_1(z: numpy.complex64) -> numpy.complex64:
    with warnings.catch_warnings(action="ignore"):
        z = numpy.complex64(z)
        lnz: numpy.complex64 = numpy.log(z)
        x: numpy.float32 = (lnz).real
        ln2: numpy.float32 = numpy.log(numpy.float32(2.0))
        result = make_complex((x) / (ln2), ((lnz).imag) / (ln2))
        return result

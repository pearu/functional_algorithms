...
def asin_0(z: complex) -> complex:
  ...
  r: float = ((_hypot_1_mx) * (sqrt_two)) if ((_hypot_1_mx) == (mn)) else ((_hypot_1_mx) * (math.sqrt(((_square_1_z) * (_square_1_z)) + (1))))
  xm1: float = (x) - (1)
  ...
  s: float = ((_hypot_2_mx) * (sqrt_two)) if ((_hypot_2_mx) == (_hypot_2_mn)) else ((_hypot_2_mx) * (math.sqrt(((_square_2_z) * (_square_2_z)) + (1))))
  a: float = (0.5) * ((r) + (s))
  ...
  smxm1: float = (s) - (xm1)
  spxm1: float = (s) + (xm1)
  real: float = math.atan2(signed_x, (y) if ((max(x, y)) >= (safe_max)) else ((math.sqrt((half_apx) * (((yy) / (rpxp1)) + (smxm1)))) if ((x) <= (1)) else ((y) * (math.sqrt(((half_apx) / (rpxp1)) + ((half_apx) / (spxm1)))))))
  ...
  x_ge_1_or_not: float = ((divide_half_yy_rpxp1) + ((0.5) * (spxm1))) if ((x) >= (1)) else (((divide_half_yy_rpxp1) + ((half_yy) / (smxm1))) if ((a) <= (1.5)) else ((a) - (1)))
  am1: float = (-(((xp1) * (xm1)) / (ap1))) if (logical_and_lt_y_safe_min_lt_x_one) else (x_ge_1_or_not)
  sq: float = math.sqrt((am1) * (ap1))
  imag: float = (((math.log(2)) + (math.log(mx))) + ((0.5) * (math.log1p((xoy) * (xoy))))) if ((mx) >= ((safe_max_opt) if (y_gt_safe_max_opt) else (safe_max))) else (((y) / (sq)) if (logical_and_lt_y_safe_min_lt_x_one) else (math.log1p((am1) + (sq))))

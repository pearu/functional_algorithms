...
def acosh_0(z: complex) -> complex:
    ...
    one: float = 1.0
    logical_and_302: bool = ((y) < ((math.sqrt(sys.float_info.min)) * (4.0))) and ((x) < (one))
    xp1: float = (x) + (one)
    xm1: float = (x) - (one)
    ...
    am1: float = (-(((xp1) * (xm1)) / (ap1))) if (logical_and_302) else (x_ge_1_or_not)
    ...
    return complex(
        (
            ...
            else (((y) / (sq)) if (logical_and_302) else (math.log1p((am1) + (sq))))
        ),
...
def acosh_1(z: float) -> float:
    ...
    sqrt_26: float = math.sqrt((z) - (constant_f1))
    return (
        ((math.log(constant_f2)) + (math.log(z)))
        if ((z) >= ((sys.float_info.max) / (constant_f2)))
        else (math.log1p((sqrt_26) * ((math.sqrt((z) + (constant_f1))) + (sqrt_26))))
    )
